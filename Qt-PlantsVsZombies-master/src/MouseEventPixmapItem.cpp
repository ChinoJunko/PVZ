...
HoverChangedPixmapItem::HoverChangedPixmapItem(const QPixmap &image) : origImage(image.copy(0, 0, image.width(), image.height() / 2)),
                                                                                 hoverImage(image.copy(0, image.height() / 2, image.width(), image.height() / 2))//图片上半为暗，下半为高光
{
    setPixmap(origImage);
    setAcceptHoverEvents(true);
...
void MoviePixmapItem::setMovie(const QString &filename)
{
    ...
    if(movieRed!=255||movieGreen!=255||movieBlue!=255){
        ...
        sourcePainter.setCompositionMode(QPainter::RasterOp_SourceAndDestination);//与模式-作为颜色蒙版
...
void MoviePixmapItem::setMovieOnNewLoop(const QString &filename, std::function<void(void)> functor)//循环
{
